...
import { getCentersReport, type ReportFilters } from "@/services/reports/reports.service";

/**
 * GET /api/projects/[projectId]/reports/centers
 * Returns per-center performance metrics including completion, delays, and approval turnaround.
 */
export async function GET(
  ...
  try {
    ...
    const report = await getCentersReport(projectId, filters);
    return NextResponse.json(report);
  } catch (error: unknown) {
    console.error("GET /api/projects/[projectId]/reports/centers error:", error);
    return NextResponse.json(
