...
import { getVolunteerReport, type ReportFilters } from "@/services/reports/reports.service";

/**
 * GET /api/projects/[projectId]/reports/volunteer
 * Returns volunteer activity aggregates with center-level breakdown.
 */
export async function GET(
  ...
  try {
    ...
    const report = await getVolunteerReport(projectId, filters);
    return NextResponse.json(report);
  } catch (error: unknown) {
    console.error("GET /api/projects/[projectId]/reports/volunteer error:", error);
    return NextResponse.json(
